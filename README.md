...
- Reply Keyboard utama permanen melalui `/start` atau `/menu`, lengkap dengan akses produk, voucher, laporan stok, deposit, panduan, informasi, dan riwayat.
- Tombol nomor 1-25 memilih produk berdasarkan urutan produk aktif, bukan berdasarkan ID database.
- Daftar produk, produk populer, dan pagination inline (10 produk per halaman).
- Pemilihan produk lewat inline button atau nomor 1-25 berdasarkan urutan global produk aktif.
...
├── keyboards/      # reply keyboard utama dan inline keyboard produk
...
Command Telegram yang tersedia untuk user adalah `/start`, `/menu`, `/products`, `/wallet`, `/orders`, dan `/help`. Bot mendaftarkan command tersebut melalui `setMyCommands` saat startup. Jika Telegram API sementara tidak dapat menerima konfigurasi command, bot tetap dijalankan dan mencatat peringatan.
