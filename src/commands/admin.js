...
function registerAdminCommands(bot) {
  ...
  bot.command('orders', adminOnly(async (ctx) => { const rows = orderService.listOrders(); const text = rows.map((o) => `#${o.id} ${o.invoice} — ${escapeHtml(o.name)} x${o.quantity} — ${money(o.total_price)} — ${o.status}`).join('\n') || 'Belum ada order.'; await ctx.reply(`<b>🧾 Order terbaru</b>\n\n${text}`, { parse_mode: 'HTML' }); }));
