...
function registerAdminCommands(bot) {
  ...
  bot.command('orders', adminOnly(async (ctx) => { const rows = orderService.listOrders(); const text = rows.map((o) => `#${o.id} ${o.invoice} — ${escapeHtml(o.name)} — ${escapeHtml(o.variant_name)} x${o.quantity} @${money(o.unit_price)} = ${money(o.total_price)} — ${o.payment_method}/${o.payment_status||o.status} — catatan: ${escapeHtml(o.note||'-')} — ${o.created_at}`).join('\n') || 'Belum ada order.'; await ctx.reply(`<b>🧾 Order terbaru</b>\n\n${text}`, { parse_mode: 'HTML' }); }));
