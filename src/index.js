...
bot.use(async(ctx,next)=>{if(!ctx.from||!ctx.callbackQuery)return next();const data=ctx.callbackQuery.data||'',sensitive=/^(paycheck:|depcheck:)/i.test(data),wait=sensitive?3000:600,key=`${ctx.from.id}:${sensitive?'payment':'callback'}`,now=Date.now(),last=rateState.get(key)||0;if(now-last<wait){await ctx.answerCbQuery('Tunggu sebentar sebelum mencoba lagi.').catch(()=>{});return;}rateState.set(key,now);if(rateState.size>5000)for(const[k,t]of rateState)if(now-t>60000)rateState.delete(k);return next();});
