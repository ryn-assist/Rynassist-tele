...
function listView(ctx, page, popular) {
  const result = productService.listProducts(page, 10, popular);
  const title = popular ? '🔥 <b>Produk Populer</b>' : '📦 <b>List Produk</b>';
  const instruction = result.items.length ? 'Pilih produk melalui tombol inline atau keyboard nomor 1-25.' : 'Belum ada produk aktif.';
  const text = `${title}\n\n${instruction}\nHalaman ${result.page}/${result.pages}`;
  return { text, extra: { parse_mode: 'HTML', ...productListKeyboard(result, ctx.from.id, popular) } };
}
async function showList(ctx, page, popular = false) {
  const view = listView(ctx, page, popular);
  await safeEdit(ctx, view.text, view.extra);
}
async function replyList(ctx, page = 1, popular = false) {
  const view = listView(ctx, page, popular);
