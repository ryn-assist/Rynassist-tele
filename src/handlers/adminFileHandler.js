...
function registerAdminFileHandlers(bot){
 ...
 bot.on('document',async ctx=>{if(!admin(ctx))return;const id=ctx.session.awaitingRestockFile;if(!id)return;const doc=ctx.message.document;if(!/\.txt$/i.test(doc.file_name||'')){return ctx.reply('❌ File harus berformat .txt.');}if(Number(doc.file_size||0)>MAX_FILE_BYTES)return ctx.reply('❌ File terlalu besar. Maksimal 1 MB.');try{const link=await ctx.telegram.getFileLink(doc.file_id);const response=await fetch(link);if(!response.ok)throw new Error('Gagal mengunduh file Telegram.');const items=lines(await response.text());let total=0;for(let i=0;i<items.length;i+=100)total+=products.addVariantStock(id,items.slice(i,i+100));const variant=products.getVariant(id);delete ctx.session.awaitingRestockFile;await ctx.reply(`✅ Restock dari file berhasil.\n${variant.product_name} — ${variant.name}\nStok masuk: ${total} item.`);await notifyRestock(ctx,variant);}catch(e){await ctx.reply(`❌ ${e.message}`);}});
 bot.command('cancel',ctx=>{if(!admin(ctx)||!ctx.session.awaitingRestockFile)return;delete ctx.session.awaitingRestockFile;return ctx.reply('Upload restock dibatalkan.');});
}
