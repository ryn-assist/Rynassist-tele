...
async function replyBalance(ctx) {
  const user = userService.getUser(ctx.from.id) || userService.upsertUser(ctx.from);
  const username = user.username ? `@${escapeHtml(user.username)}` : '-';
  return ctx.reply(`💰 <b>SALDO RYNASSIST</b>\n\n👤 ID : <code>${ctx.from.id}</code>\n🏷️ Username : ${username}\n💵 Saldo : <b>${money(user.balance)}</b>`, { parse_mode: 'HTML' });
}
...
function registerAccountHandlers(bot) {
  ...
  bot.hears('💰 Cek Saldo',replyBalance);bot.command('saldo',replyBalance);
  ...
  bot.action(/^depcancel:(DEP-[A-Za-z0-9-]+)$/,async ctx=>{try{const ok=await depositService.cancelDeposit(ctx.match[1],ctx.from.id);await ctx.answerCbQuery(ok?'Deposit dibatalkan.':'Deposit tidak dapat dibatalkan.',{show_alert:true});if(ok)await ctx.reply('❌ Deposit dibatalkan. QRIS tersebut tidak berlaku lagi.');}catch(e){await ctx.answerCbQuery(e.message,{show_alert:true});}});
}
