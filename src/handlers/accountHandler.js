...
async function replyOrders(ctx) { const orders = orderService.userOrders(ctx.from.id); const text = orders.map((o) => `<code>${o.invoice}</code> — ${escapeHtml(o.name)}${o.variant_name ? ` — ${escapeHtml(o.variant_name)}` : ''} x${o.quantity} — ${money(o.total_price)} — ${o.status}`).join('\n') || 'Belum ada transaksi.'; return ctx.reply(`<b>🧾 Riwayat Transaksi</b>\n\n${text}`, { parse_mode: 'HTML' }); }
