...
function createProduct({ code, name, price, description }) {
  ...
  return getDb().transaction(() => { const id = getDb().prepare('INSERT INTO products(code,name,price,description) VALUES(?,?,?,?)').run(requireProductCode(code), cleanName, cleanPrice, cleanDescription).lastInsertRowid; getDb().prepare('INSERT INTO product_variants(product_id,name,price) VALUES(?,?,?)').run(id, cleanName, cleanPrice); return id; }).immediate();
}
...
function listVariants(productId, activeOnly = false) { return getDb().prepare(`SELECT v.*,COUNT(CASE WHEN s.status='available' THEN 1 END) available_stock,COUNT(s.id) total_stock FROM product_variants v LEFT JOIN stock_items s ON s.variant_id=v.id WHERE v.product_id=? ${activeOnly?'AND v.is_active=1':''} GROUP BY v.id ORDER BY v.id`).all(requirePositiveInteger(productId,'ID produk')); }
function getVariant(id, activeOnly = false) { return getDb().prepare(`SELECT v.*,p.name product_name,p.code,p.description,p.is_active product_active,COUNT(CASE WHEN s.status='available' THEN 1 END) available_stock,COUNT(s.id) total_stock FROM product_variants v JOIN products p ON p.id=v.product_id LEFT JOIN stock_items s ON s.variant_id=v.id WHERE v.id=? ${activeOnly?'AND v.is_active=1 AND p.is_active=1':''} GROUP BY v.id`).get(requirePositiveInteger(id,'ID varian')); }
function createVariant(productId, name, price) { const id=requirePositiveInteger(productId,'ID produk'); if(!getDb().prepare('SELECT 1 FROM products WHERE id=?').get(id)) throw new Error('Produk tidak ditemukan.'); return getDb().prepare('INSERT INTO product_variants(product_id,name,price) VALUES(?,?,?)').run(id,requireText(name,'Nama varian',100),requireNonNegativeInteger(price,'Harga')).lastInsertRowid; }
function updateVariant(id, field, value) { const variantId=requirePositiveInteger(id,'ID varian'); const validators={name:(v)=>requireText(v,'Nama varian',100),price:(v)=>requireNonNegativeInteger(v,'Harga'),is_active:(v)=>{const n=Number(v);if(![0,1].includes(n))throw new Error('is_active hanya boleh 0 atau 1.');return n;}}; if(!validators[field]) throw new Error('Field varian hanya name, price, atau is_active.'); return getDb().prepare(`UPDATE product_variants SET ${field}=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(validators[field](value),variantId); }
function deleteVariant(id) { return updateVariant(id,'is_active',0); }
function addVariantStock(variantId, contents) { const variant=getVariant(variantId); if(!variant) throw new Error('Varian tidak ditemukan.'); if(!Array.isArray(contents)||!contents.length||contents.length>100) throw new Error('Restock harus berisi 1-100 item.'); const clean=contents.map(v=>requireText(v,'Item stok',3000)); if(new Set(clean).size!==clean.length) throw new Error('Item stok duplikat ditemukan dalam input.'); return getDb().transaction(()=>{const exists=getDb().prepare('SELECT 1 FROM stock_items WHERE variant_id=? AND content=?'); const insert=getDb().prepare('INSERT INTO stock_items(product_id,variant_id,content) VALUES(?,?,?)'); for(const content of clean){if(exists.get(variant.id,content))throw new Error('Item stok sudah pernah tersimpan untuk varian ini.');insert.run(variant.product_id,variant.id,content);}return clean.length;}).immediate(); }
