...
function getActiveProductByPosition(position) {
  const safePosition = requirePositiveInteger(position, 'Nomor produk');
  if (safePosition > 25) return undefined;
  return getDb().prepare(`${PRODUCT_SELECT} WHERE p.is_active=1 GROUP BY p.id ORDER BY p.id ASC LIMIT 1 OFFSET ?`).get(safePosition - 1);
}
...
function activeStockSummary() {
  return getDb().prepare(`${PRODUCT_SELECT} WHERE p.is_active=1 GROUP BY p.id ORDER BY p.id`).all();
}
module.exports = { listProducts, getProduct, getProductByCode, getActiveProductByPosition, createProduct, updateProduct, deleteProduct, addStock, stockSummary, activeStockSummary };
