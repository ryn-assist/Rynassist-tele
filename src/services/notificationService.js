...
async function transaction(bot,data){
  const chatId=config.notificationChatId;if(!chatId)return false;
  ...
  try{await bot.telegram.sendMessage(chatId,text,{parse_mode:'HTML',disable_web_page_preview:true});return true;}catch(e){console.warn('Notifikasi channel gagal:',e.message);return false;}
}
