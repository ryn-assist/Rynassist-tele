...
function migrateVariants(database) {
  ...
  const additions = {
    product_variants: [['code', 'TEXT'], ['description', "TEXT NOT NULL DEFAULT ''"]],
  ...
  for (const [table, columns] of Object.entries(additions)) {
    for (const [column, definition] of columns) if (!hasColumn(database, table, column)) database.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`);
  }
